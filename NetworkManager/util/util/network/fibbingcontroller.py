...
class FibbingController(_node.Host, L3Router):
    ...
    def __init__(self, name, cfg_path=None, quiet=False, *args, **kwargs):
        ...
        self.cfg_path = "/tmp/%s.cfg" % self.name if not cfg_path else cfg_path
    ...
    def dump_cfg_info(self):
        ...
        for itf in connected_intfs:
            ...
            hello = self.config_params.get('hello_interval')
            dead = self.config_params.get('dead_interval')
            # Derived from Router which is connected to
            cfg.set(itf.name, 'hello_interval', hello)
            cfg.set(itf.name, 'dead_interval',dead)
